...
export function ColorModePicker() {
  ...
  return <View style={styles.section}>
    <Text style={styles.title} accessibilityRole="header">{t('Appearance')}</Text>
    ...
    <View style={styles.choices} accessibilityRole="radiogroup" accessibilityLabel={t('Appearance')}>
      ...
      >
        <Ionicons name={item === 'light' ? 'sunny-outline' : 'moon-outline'} size={21} color={item === mode ? '#ffb000' : '#96969d'} />
        <Text style={[styles.choiceText, item === mode && styles.selectedText]}>{t(item === 'light' ? 'Day' : 'Night')}</Text>
        {saving === item ? <ActivityIndicator color="#ffb000" /> :
          <Ionicons name={item === mode ? 'checkmark-circle' : 'ellipse-outline'} size={21} color={item === mode ? '#ffb000' : '#96969d'} />}
      </TouchableOpacity>)}
    </View>
  </View>;
}
...
const styles = adaptiveStyles(StyleSheet.create({
  ...
  choice: { flex: 1, minHeight: 56, borderRadius: 14, borderWidth: 1, borderColor: '#39393e', backgroundColor: '#151518', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 9, padding: 12 },
  selected: { borderColor: '#96701c', backgroundColor: '#211c11' },
  choiceText: { color: '#eeeef0', fontSize: 16, fontWeight: '600' },
