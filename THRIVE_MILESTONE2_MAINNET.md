...
**Status:** prepared, not yet deployed or submitted

**Scope:** the Hedera Mainnet wallet and checkout path in the public multi-chain Opago hackathon repository

This document is intentionally incomplete until real Mainnet consensus evidence exists. Never replace a missing value with a Testnet link, an intended transaction, or a manually invented identifier.
...
Pending deployment. After verification, copy `hashscanContractUrl` from `deployments/hedera-mainnet.json` without editing the identifier manually.
...
- [ ] The deployment account has at least the documented 30 HBAR safety reserve.
- [ ] `npm run contract:preflight:mainnet` passes immediately before deployment.
- [ ] Opago explicitly approves the exact runtime hash and real-HBAR deployment.
- [ ] `npm run contract:deploy:mainnet` reaches Mainnet consensus successfully.
- [ ] `npm run contract:verify:mainnet` confirms Mirror Node bytecode and Sourcify status.
- [ ] `deployments/hedera-mainnet.json` contains only script-derived public evidence.
