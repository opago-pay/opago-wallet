...
const { loadPaymentEndpoint } = require('../lib/payment-endpoint.ts');
...
test('recognition validates an invoice signature, amount, expiry and network without SDK preparation', async () => {
  ...
  for (const invalid of [raw.slice(0,-1)+(raw.at(-1)==='x'?'y':'x'), invoice(20, 1), invoice(20, undefined, {network:'bc'}), 'https://example.com', 'hello']) {
...
test('a standard LNURL endpoint is fetched once before classifying its response', async () => {
  const raw = bech32.encode('lnurl', bech32.toWords(Buffer.from('https://pay.example/lnurl')), 2000);
  const previous = global.fetch; const requests = [];
  global.fetch = async url => {
    requests.push(String(url));
    return new Response(JSON.stringify({ tag: 'payRequest', minSendable: 20_000, maxSendable: 20_000,
      callback: 'https://pay.example/callback', metadata: '[["text/plain","Test"]]' }),
    { headers: { 'content-type': 'application/json' } });
  };
  try {
    const result = await recognizePayment(raw);
    assert.equal(result.kind, 'lightning');
    assert.equal(result.amount, '20');
    assert.equal((await loadPaymentEndpoint('https://pay.example/lnurl')).kind, 'lnurl');
    assert.deepEqual(requests, ['https://pay.example/lnurl']);
  } finally { global.fetch = previous; }
});
